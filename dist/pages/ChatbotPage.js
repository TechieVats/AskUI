...
class ChatbotPage {
    ...
    async rateLastMessage(isHelpful) {
        const lastMessage = await this.getLastBotResponse();
    ...
    async getLastMessage() {
        ...
        await Promise.race([
            ...
            this.userMessage.waitFor({ state: 'visible', timeout: 30000 })
        ]);
        // Get the last message of either type
        const lastBotMessage = this.botMessage.last();
        const lastUserMessage = this.userMessage.last();
        // Check which one is more recent by comparing their positions
        const botBox = await lastBotMessage.boundingBox();
        const userBox = await lastUserMessage.boundingBox();
        if (!botBox || !userBox) {
            throw new Error('Could not determine message position');
        }
        // Return the message that appears later (higher y-coordinate)
        return botBox.y > userBox.y ? lastBotMessage : lastUserMessage;
    }
}
exports.ChatbotPage = ChatbotPage;
//# sourceMappingURL=ChatbotPage.js.map